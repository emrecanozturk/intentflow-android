...
IntentFlow is a workflow-first Kotlin architecture for Android. Treat feature behavior as the source of truth, then adapt Jetpack Compose or Android Views to that behavior.

## Read First

- Start with `.ai/agent-context.md` for the compact repository map.
- For AI-mode feature work, read the feature `.intentflow.yaml` manifest before Kotlin implementation files.
...
- Avoid reading generated Android Studio project internals unless the task is explicitly about the demo app project.

## Architecture Rules

- Update the manifest before changing AI-mode state, intent, event, effect, output, or route contracts.
- Keep reducers pure. Reducers must not call networking, persistence, analytics, timers, UI APIs, or launch coroutines.
...
- Keep Jetpack Compose views and Android View adapters as adapters over projected state.
...
- Prefer `./gradlew :intentflow-generator:run --args="ai-context <manifest> --tool codex"` over loading the whole repository.
