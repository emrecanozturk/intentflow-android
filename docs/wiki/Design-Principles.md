...
IntentFlow exists because many Android features are no longer simple screens. They are workflows with async work, cancellation, navigation, recovery, parent communication, and test expectations.

The architecture starts from product behavior, then lets UI and infrastructure adapt to that behavior.

## The Problem

Common Android patterns often begin with a screen or layer:
...
| UI is an adapter | Jetpack Compose and Android Views render projections and send intents. |
...
- a manifest that disagrees with Kotlin types
