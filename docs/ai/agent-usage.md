...
Claude should start from `CLAUDE.md`. Path-scoped rules in `.claude/rules` keep Kotlin and documentation guidance separate, so Claude does not need every rule for every task.
...
Gemini should start from `GEMINI.md`. `.geminiignore` keeps noisy build, Git, media, and Android Studio user-state files out of normal context.
...
- `.github/instructions/intentflow-kotlin.instructions.md`
- `.github/instructions/intentflow-docs.instructions.md`

Copilot review should focus on:

- manifest and Kotlin contract alignment
- reducer purity
- effect cancellation
- route/output typing
